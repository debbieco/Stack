...
def StartBoxCoordsLeft(topBox,d):
    ans=[[coord[0]-300,coord[1]-325] for coord in topBox]
    ans.append([d,d])
    return ans
    ...
    while gameNotOver:    
        ...
        if Draw.hasNextKeyTyped(): #If the user presses any key
            ...
            if stackList[-1][-1][0]==stackList[-1][-1][1]: 
                #If boxes overlap (coming from left up) or exact
                ...
                #If boxes overlap in another way
                elif stackList[-2][3][0]<stackList[-1][4][0]<stackList[-2][2][0]:
                    moveStack(stackList)
                    #Next line--> What are the coordinates of the new stackList[-1], append them to stackList
                ...
                #If boxes overlap in another way
                elif stackList[-2][2][0]<stackList[-1][6][0]<stackList[-2][1][0]:
                    moveStack(stackList)
                    #Next line--> What are the coordinates of the new stackList[-1], append them to stackList
                    stackList[-1]=[[stackList[-1][0][0], stackList[-1][0][1]],\
                               [stackList[-1][1][0], stackList[-1][1][1]],\
